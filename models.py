...
task_material_association = Table('task_material_association', Base.metadata,
                                  Column('task_id', Integer, ForeignKey('tasks.id'), primary_key=True),
                                  Column('material_id', Integer, ForeignKey('materials.id'), primary_key=True)
                                  )

task_protection_association = Table('task_protection_association', Base.metadata,
                                    Column('task_id', Integer, ForeignKey('tasks.id'), primary_key=True),
                                    Column('protection_id', Integer, ForeignKey('protections.id'), primary_key=True)
                                    )

task_consumable_association = Table('task_consumable_association', Base.metadata,
                                    Column('task_id', Integer, ForeignKey('tasks.id'), primary_key=True),
                                    Column('consumable_id', Integer, ForeignKey('consumables.id'), primary_key=True)
                                    )

project_protection_association = Table('project_protection_association', Base.metadata,
                                       Column('project_id', Integer, ForeignKey('projects.id'), primary_key=True),
                                       Column('protection_id', Integer, ForeignKey('protections.id'), primary_key=True)
                                       )

project_material_association = Table('project_material_association', Base.metadata,
                                     Column('project_id', Integer, ForeignKey('projects.id'), primary_key=True),
                                     Column('material_id', Integer, ForeignKey('materials.id'), primary_key=True)
                                     )

project_consumable_association = Table('project_consumable_association', Base.metadata,
                                       Column('project_id', Integer, ForeignKey('projects.id'), primary_key=True),
                                       Column('consumable_id', Integer, ForeignKey('consumables.id'), primary_key=True)
                                       )

project_project_association = Table('project_project_association', Base.metadata,
                                    Column('project_id', Integer, ForeignKey('projects.id'), primary_key=True),
                                    Column('prerequisite_project_id', Integer, ForeignKey('projects.id'),
                                           primary_key=True)
                                    )

protocol_protection_association = Table('protocol_protection_association', Base.metadata,
                                        Column('protocol_id', Integer, ForeignKey('protocols.id')),
                                        Column('protection_id', Integer, ForeignKey('protections.id')),
                                        extend_existing=True
                                        )

protocol_material_association = Table('protocol_material_association', Base.metadata,
                                      Column('protocol_id', Integer, ForeignKey('protocols.id')),
                                      Column('material_id', Integer, ForeignKey('materials.id')),
                                      extend_existing=True
                                      )

protocol_consumable_association = Table('protocol_consumable_association', Base.metadata,
                                        Column('protocol_id', Integer, ForeignKey('protocols.id')),
                                        Column('consumable_id', Integer, ForeignKey('consumables.id')),
                                        extend_existing=True
                                        )

protocol_protocol_association = Table('protocol_protocol_association', Base.metadata,
                                      Column('protocol_id', Integer, ForeignKey('protocols.id'), primary_key=True),
                                      Column('prerequisite_protocol_id', Integer, ForeignKey('protocols.id'),
                                             primary_key=True)
                                      )
...
class BaseActivity(BaseModel):
    ...
    def __init__(self, name, goal='', principle='', protocol_text='', time_required=0, time_required_unit='seconds',
                 funding=0, benefits=0, time_to_sell=0, time_to_sell_unit='seconds'):
        self.name = name
        self.goal = goal
        ...
        super().__init__()
        self.save(self.__class__.__name__[0])
...
class Task(db.Model):
    ...
    prerequisites = db.relationship(
        ...
        secondaryjoin=(task_task_association.c.prerequisite_task_id == remote(
            task_task_association.c.prerequisite_task_id)),
...
class TaskPrerequisite(db.Model):
    ...
    task = db.relationship('Task', foreign_keys=[task_id],
                           backref=db.backref('task_prerequisites', cascade='all, delete-orphan'))
...
class Project(BaseActivity):
    ...
    prerequisites = db.relationship(
        ...
        # Ajoutez cette ligne pour utiliser remote_side
    )

    def __init__(self, name, due_date=None, planned_date=None, urgency=0.0, impact=0.0, resources=0.0, complexity=0.0,
                 alignment=0.0, priority=0.0, **kwargs):
